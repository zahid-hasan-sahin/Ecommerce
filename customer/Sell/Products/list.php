		...
		<table id="dash-table" class="table table-striped table-bordered table-hover " style="font-size:12px" cellspacing="0">
			...
			<tbody>
				...
				$query = "SELECT * FROM `tblpromopro` pr , `tblproduct` p , `tblcategory` c
           					 WHERE pr.`PROID`=p.`PROID` AND  p.`CATEGID` = c.`CATEGID` and p.userid = '" . $userId . "'";
				$mydb->setQuery($query);
				...
				foreach ($cur as $result) {
					...
					echo '<td style="padding:0px;">
							...
							<img  title="' . $result->CATEGORIES . '" style="width:100px;height:50px;padding:0;"  src="' . web_root . 'customer/sell/products/' . $result->IMAGES . '">
							</a></td>';
					// echo '<td><a title="edit" href="'.web_root.'customer/sell/products/index.php?view=edit&id='.$result->PROID.'"><i class="fa fa-pencil "></i> ' . $result->PROMODEL.'</a></td>';
					echo '<td><a title="edit" href="' . web_root . 'customer/sell/products/index.php?view=edit&id=' . $result->PROID . '"><i class="fa fa-pencil "></i>' . $result->CATEGORIES . '</a></td>';
					...
					// 	echo
					// 	 '<td align="left">
					// <a href="'.web_root.'customer/sell/products/controller.php?action=edit&id='.$result->PROID.'&stats='.$stats.'" class="btn btn-primary btn-xs">'.$stats.'</a>
...
<div class="modal fade" id="productmodal" tabindex="-1">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<form action="<?php echo web_root; ?>customer/sell/products/controller.php?action=photos" enctype="multipart/form-data" method="post">
